...
### Compares multiclass logistic regression vs small MLP ###
...
def row_in_split(row_idx, split, test_fraction=0.2):
    ...
    Args:
      row_idx: 1-based row index within csv data rows (not header)
      split: "train" or "test"
    ...
    Returns:
      Boolean
    """
    digest = hashlib.md5(str(row_idx).encode("utf-8")).hexdigest()
...
def parse_training_row(row):
    ...
    Args:
      row: dict from training csv

    Returns:
      Parsed dict or None if bad row
    """
    try:
...
def compute_train_stats(in_csv, test_fraction=0.2):
    """One streaming pass over train split to compute feature mean/std/class counts"""
    n = 0
...
def save_model_results(eval_results, model_name, exp_name, in_csv, train_stats, config, run_dir, timestamp):
    """
